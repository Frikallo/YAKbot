...
def genSoundFromImage(
    file, output="sound.wav", duration=5.0, sampleRate=44100.0, maxfreq=22000
):
    ...
    for frame in range(max_frame):
        ...
        for step in range(steppingSpectrum):
            ...
            for freq in range(nextFreq, currentFreq, 1000):  # substep of 1000 Hz works
                signalValue += intensity * math.cos(
                    freq * 2 * math.pi * float(frame) / float(sampleRate)
                )
                count += 1
        if count == 0:
            count = 1
...
outfile = "out.wav"
genSoundFromImage(
    file=infile, output=outfile, duration=5.0, sampleRate=44100.0, maxfreq=22000
)
