...
from taming.data.conditional_builder.object_center_points_builder import (
    CoordinatesCenterPointsConditionalBuilder,
)
...
def get_z_indices_crop_out(z_indices: Tensor, predict_x: int, predict_y: int) -> Tensor:
    ...
    cut_out_1 = z_indices[:, y0:predict_y, x0 : x0 + first_stage_factor].reshape(
        (no_images, -1)
    )
...
def sample(
    ...
    dataset: AnnotatedObjectsDataset,
    conditional_builder: CoordinatesCenterPointsConditionalBuilder,
    no_samples: int,
    temperature: float,
    top_k: int,
) -> Tensor:
    ...
    annotations = [
        a._replace(category_no=dataset.get_category_number(a.category_id))
        for a in annotations
    ]

    recompute_conditional = any(
        (desired_resolution[0] > trained_on_res, desired_resolution[1] > trained_on_res)
    )
    if not recompute_conditional:
        ...
        for predict_y, predict_x in tqdm(
            product(range(y_max), range(x_max)),
            desc="sampling_image",
            total=x_max * y_max,
        ):
            ...
            conditional_indices = conditional_builder.build(
                annotations, crop_coordinates
            )
            c_indices = conditional_indices.to(device).repeat(no_samples, 1)
            new_index = model.sample(
                z_indices,
                c_indices,
                steps=1,
                temperature=temperature,
    ...
    scene_graph = conditional_builder.build(annotations, (0.0, 0.0, 1.0, 1.0))
...
if __name__ == "__main__":
    ...
    outdir = Path(opt.outdir).joinpath(
        f"{global_step:06}_{opt.top_k}_{opt.temperature}"
    )
    ...
    for batch_no, batch in p_bar_1:
        ...
        for i, annotations in tqdm(
            enumerate(batch["annotations"]),
            desc="within_batch",
            ...
            imgs = sample(
                model,
                annotations,
                dsets.datasets["validation"],
                conditional_builder,
                opt.n_samples_per_layout,
                opt.temperature,
                opt.top_k,
            )
            save_image(
                imgs,
                outdir.joinpath(f"{batch_no:04}_{i:02}.png"),
                n_row=opt.n_samples_per_layout + 1,
            )
