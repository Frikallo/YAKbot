...
def save_image(x, path):
    ...
    assert c == 3
    x = (
        ((x.detach().cpu().numpy().transpose(1, 2, 0) + 1.0) * 127.5)
        .clip(0, 255)
        .astype(np.uint8)
    )
...
def run_conditional(model, dsets, outdir, top_k, temperature, batch_size=1):
    ...
    for start_idx in trange(0, len(dset) - batch_size + 1, batch_size):
        ...
        for i in range(x.shape[0]):
            save_image(
                x[i], os.path.join(outdir, "originals", "{:06}.png".format(indices[i]))
            )
        ...
        for i in range(xrec.shape[0]):
            save_image(
                xrec[i],
                os.path.join(outdir, "reconstructions", "{:06}.png".format(indices[i])),
            )
        ...
        for i in range(xsample.shape[0]):
            save_image(
                xsample[i],
                os.path.join(outdir, "samples", "{:06}.png".format(indices[i])),
            )
...
def get_parser():
    ...
    parser.add_argument(
        "--outdir", required=True, type=str, help="Where to write outputs to.",
    )
    parser.add_argument(
        "--top_k", type=int, default=100, help="Sample from among top-k predictions.",
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    outdir = os.path.join(
        opt.outdir, "{:06}_{}_{}".format(global_step, opt.top_k, opt.temperature)
    )
