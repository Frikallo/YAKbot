...
def single_image_to_torch(x, permute=True):
    ...
    x = np.array(x)
    x = torch.FloatTensor(x / 255.0 * 2.0 - 1.0)[None, ...]
    if permute:
        x = x.permute(0, 3, 1, 2)
    return x
...
def run_conditional(model, dsets):
    ...
    start_index = st.sidebar.number_input(
        "Example Index (Size: {})".format(len(dset)),
        value=0,
        min_value=0,
    ...
    scale_factor = st.sidebar.slider(
        "Scale Factor", min_value=0.5, max_value=4.0, step=0.25, value=1.00
    )
    if scale_factor != 1.0:
        x = torch.nn.functional.interpolate(
            x, scale_factor=scale_factor, mode="bicubic"
        )
        c = torch.nn.functional.interpolate(
            c, scale_factor=scale_factor, mode="bicubic"
        )
        ...
        for i in range(start_i, cshape[2] - 0):
            ...
            for j in range(start_j, cshape[3] - 0):
                ...
                if (i * cshape[3] + j) % update_every == 0:
                    xstart = model.decode_to_img(
                        idx[:, : cshape[2], : cshape[3]], cshape,
                    )
                    ...
                    if animate:
                        writer.append_data(
                            (xstart[0] * 255).clip(0, 255).astype(np.uint8)
                        )
