...
def sample_unconditional(
    ...
    c_indices = repeat(torch.tensor([model.sos_token]), "1 -> b 1", b=batch_size).to(
        model.device
    )  # sos token
    t1 = time.time()
    index_sample = sample_with_past(
...
def run(
    ...
    if not unconditional:
        ...
        print(
            f"Running in unconditional sampling mode, producing {num_samples} samples."
        )
        for n, bs in tqdm(enumerate(batches), desc="Sampling"):
            if bs == 0:
                break
            logs = sample_unconditional(
                model, batch_size=bs, temperature=temperature, top_k=top_k, top_p=top_p
            )
...
if __name__ == "__main__":
    ...
    logdir = os.path.join(
        logdir,
        "samples",
        f"top_k_{opt.top_k}_temp_{opt.temperature:.2f}_top_p_{opt.top_p}",
        f"{global_step}",
    )
