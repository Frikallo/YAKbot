...
class ObjectsCenterPointsConditionalBuilder:
    def __init__(
        ...
        use_additional_parameters: bool,
    ):
    ...
    def token_pair_from_bbox(self, bbox: BoundingBox) -> Tuple[int, int]:
        return (
            self.tokenize_coordinates(bbox[0], bbox[1]),
    ...
    def inverse_build(
        ...
        if self.encode_crop:
            crop_coordinates = self.bbox_from_token_pair(
                conditional_list[-2], conditional_list[-1]
            )
    ...
    def plot(
        ...
        for (representation, (x, y)), color in zip(description, cycle(COLOR_PALETTE)):
            ...
            label = (
                label_for_category_no(ann.category_no)
                + " "
    ...
    def build(
        self,
        annotations: List,
        crop_coordinates: Optional[BoundingBox] = None,
        ...
        if self.encode_crop:
            ...
            annotations = rescale_annotations(
                annotations, crop_coordinates, horizontal_flip
            )
            extra = []
