...
class ObjectsBoundingBoxConditionalBuilder(ObjectsCenterPointsConditionalBuilder):
    ...
    def inverse_build(
    ...
    ) -> Tuple[List[Tuple[int, BoundingBox]], Optional[BoundingBox]]:
        ...
        return (
            [
                (
                    object_triple[0],
                    self.bbox_from_token_pair(object_triple[1], object_triple[2]),
                )
    ...
    def plot(
        ...
        for (representation, bbox), color in zip(description, cycle(COLOR_PALETTE)):
            ...
            class_label = (
                label_for_category_no(annotation.category_no)
                + " "
                + additional_parameters_string(annotation)
            )
            bbox = absolute_bbox(bbox, width, height)
            draw.rectangle(bbox, outline=color, width=line_width)
            draw.text(
                (bbox[0] + line_width, bbox[1] + line_width),
                class_label,
                anchor="la",
                fill=BLACK,
                font=font,
