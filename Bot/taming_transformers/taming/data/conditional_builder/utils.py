...
def intersection_area(rectangle1: BoundingBox, rectangle2: BoundingBox) -> float:
    ...
    x_overlap = max(
        0.0, min(rectangle1[2], rectangle2[2]) - max(rectangle1[0], rectangle2[0])
    )
    y_overlap = max(
        0.0, min(rectangle1[3], rectangle2[3]) - max(rectangle1[1], rectangle2[1])
    )
...
def absolute_bbox(
    relative_bbox: BoundingBox, width: int, height: int
...
def filter_annotations(
    annotations: List[Annotation], crop_coordinates: BoundingBox
) -> List:
...
def additional_parameters_string(annotation: Annotation, short: bool = True) -> str:
    ...
    string = ""
    if not (
        annotation.is_group_of
        or annotation.is_occluded
        or annotation.is_depiction
        or annotation.is_inside
    ):
        return string
