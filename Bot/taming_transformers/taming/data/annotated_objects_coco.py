...
def load_categories(category_json: Iterable) -> Dict[str, Category]:
    return {
        str(cat["id"]): Category(
            id=str(cat["id"]), super_category=cat["supercategory"], name=cat["name"]
        )
...
def load_annotations(
    ...
    for ann in tqdm(
        chain(*annotations_json), f"Loading {split} annotations", total=total
    ):
...
class AnnotatedObjectsCoco(AnnotatedObjectsDataset):
    def __init__(self, use_things: bool = True, use_stuff: bool = True, **kwargs):
        ...
        annotations = load_annotations(
            ...
            self.get_category_number,
            self.split,
        )
        self.annotations = self.filter_object_number(
            annotations,
    ...
    def get_image_path(self, image_id: str) -> Path:
        return self.paths["files"].joinpath(
            self.image_descriptions[str(image_id)].file_name
        )
