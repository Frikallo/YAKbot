...
class BaseWithDepth(Dataset):
    ...
    def __init__(
        self, config=None, size=None, random_crop=False, crop_size=None, root=None
    ):
        self.config = config
...
class RINTrainWithDepth(ImageNetTrainWithDepth):
    def __init__(self, config=None, size=None, random_crop=True, crop_size=None):
        sub_indices = (
            "30-32, 33-37, 151-268, 281-285, 80-100, 365-382, 389-397, 118-121, 300-319"
        )
        super().__init__(
            config=config,
            size=size,
...
class DRINExamples(Dataset):
    def __init__(self):
        self.preprocessor = get_preprocessor(
            size=256, additional_targets={"depth": "image"}
        )
        with open("data/drin_examples.txt", "r") as f:
            relpaths = f.read().splitlines()
        self.image_paths = [
            os.path.join("data/drin_images", relpath) for relpath in relpaths
        ]
        self.depth_paths = [
            os.path.join("data/drin_depth", relpath.replace(".JPEG", ".png"))
...
class ImageNetScale(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.up_factor is None:
            ...
            lr = imscale(
                image, self.up_factor, keepshapes=True, keepmode=self.keep_mode
            )
