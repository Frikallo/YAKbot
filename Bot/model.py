...
class Model(pl.LightningModule):
    ...
    def __init__(self, args=None, indices=None, indices_data=None):
        ...
        self.tokenizer = GPT2Tokenizer.from_pretrained(
            "gpt2", cache_dir=self.hparams.logdir
        )
    ...
    def compute_loss(self, x, y, ctx=None):
        ...
        zipped = [
            zip(masks, labels) for masks, labels in zip(inputs.attention_mask, labels)
        ]
        labels = np.array(
            [
                [-100 if mask == 0 else token for mask, token in mask_tokens]
                for mask_tokens in zipped
            ]
        )
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.AdamW(self.parameters(), lr=self.hparams.lrate)
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer=optimizer, T_max=self.hparams.tmax
        )
